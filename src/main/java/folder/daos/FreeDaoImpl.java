...
public class FreeDaoImpl implements FreeDAO {
	...
	public List < Animal> lister() {
		 ...
		 List <  Animal > pets = new ArrayList < >();
		    ...
		    Animal pet = null;
		 /*
		 	if (options[0]=="all" && options[1]=="all") {
		 		final String SQL_SELECT_ALL = "SELECT * FROM animal;";
			    try {
			    ...
			    //<%@ include file='resultPet.jsp' %>
		 		...
		 		final String SQL_SELECT_ALL = "SELECT * FROM animal where statut=?;";
		 		String statut=options[1];
			    try {
			        
			        connexion = daoFactory.getConnection();
			        preparedStatement = initRequestPrepare( connexion, SQL_SELECT_ALL, statut );
			        resultSet = preparedStatement.executeQuery();
		 		...
		 		final String SQL_SELECT_ALL = "SELECT * FROM animal where type=?";
		 		String type=options[0];
			    try {
			        
			        connexion = daoFactory.getConnection();
			        preparedStatement = initRequestPrepare( connexion, SQL_SELECT_ALL, type );
			        resultSet = preparedStatement.executeQuery();
		 		...
		 		final String SQL_SELECT_ALL = "SELECT * FROM animal where type=? and statut=?;";
		 		String type=options[0];
		 		String statut=options[1];
			    try {
			        
			        connexion = daoFactory.getConnection();
			        preparedStatement = initRequestPrepare( connexion, SQL_SELECT_ALL,type,statut);
			        resultSet = preparedStatement.executeQuery();
