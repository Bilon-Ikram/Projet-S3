...
<body onload="showChien()">
	...
	<div class="inner-content">
		...
		<h5 class="text-center" style="color:green;">Mes réservations</h5>
		<form id="form" method="post" action='Reserv?action=allRes&cin=<%= session.getAttribute("cinU")%>'>
     ...
     <div class="search-result">
     	<c:if test="${firsTime=='vrai'}">
				<h5 style="margin-top:1.3em;text-align:center;">Veuillez remplir le formulaire pour visualiser vos réservations</h5>
			</c:if>
		...
		<table class="table table-bordered"
			style="width: 60%; margin-left: auto; margin-right: auto; font-size: 0.94em;margin-top:2em;">
				...
				<tr>
							...
							<div class="modal-content">
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-danger">
										<a style="text-decoration:none;color:white;" href='Reserv?action=canRes&cin=<%= session.getAttribute("cinU")%>&id=<c:out value="${petRes.getPet().getId()}" />&statut=<c:out value="${petRes.getRes().getStatutRes()}" />'>Oui</a>
									</button>
					...
					<td style="text-align: center;">
						...
						<c:if test='${petRes.getRes().getStatutRes()=="Payee"}'>
							...
							<a style="text-decoration:none;color:white;" href="Pets?action=edit&id=<c:out value='${pet.getId()}' />">Voir facture</a>
							</button>
						</c:if>
					</td>
		...
		<c:if test='${status=="En cours" && all.size()!=0}'>
			<div style="text-align:center;margin: 10px 35px;font-size:16px;">
			
				<B>Note :</B> Veuillez se présenter à notre local situé à <B style="font-size:14px;">Avenue Moulay Yousef 48,Rabat</B> maximum
						2 jours après votre réservation afin de compléter votre achat. Sinon elle sera annulée !	
